...
from texture_utils import obj_vv,obj_vt,obj_fv,obj_ft,fv2norm,cv2_triangle
...
def get_texture_SMPL(fname_img, fname_obj, fname_pkl, npath, obj_name, template_obj):
    ...
    vuv = texture_utils.p3d_p2d(vv, rmat, tvec, cameraMatrix).reshape(-1,3)
    ...
    # import pdb
    # pdb.set_trace()
    cv2.imwrite(os.path.join(npath, 'test_img.png'), timg)
    ...
    # pdb.set_trace()
    cv2.imwrite(os.path.join(npath, 'test_texture.png'), texture)
    ...
    # pdb.set_trace()
    cv2.imwrite(os.path.join(npath, 'visible_f_test.png'), timg)  
